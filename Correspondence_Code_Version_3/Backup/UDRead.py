...
def Dataframe(path_nist,path_ud,nist_max):
    # read csv
    ...
    df_nist = df_nist.values[:nist_max]
    df_ud = df_ud.values
...
def Correct_Config(jth,config,mul,new_config,list_config,list_term,list_number,count,Final=False):
    ...
    # print(jth,n_prev,n,int(abs(n_prev-n)-1),n_prev-n,conf)
    if abs(n_prev-n)>1:
    ...
    if Final==True:
        ...
        # print(jth,list_number[idx_dummy[0]],count[idx_dummy[0]],mul)
...
def Corrected_Config(df_ud,df_nist,ManualCorrection=False):
    ...
    for j in range(len(df_ud)):
        ...
        if jf==2:
            ...
            # config=config1_ao if de1<=de2 else config2_ao
...
def FinalSortE(data):
    data_final = []
    energyu = np.char.replace(data[:,11],"-","0.0").astype(float)
    ...
    confign[idxx],termn[idxx] = data[:,7][idxx],data[:,9][idxx]
...
def FinalSortT(data):
    ...
    config[idxx],term[idxx],energyn[idxx] = data[:,7][idxx],data[:,9][idxx],data[:,11][idxx]
    ...
    # sort = np.array([float(data_final[i][:,11][-1]) for i in range(len(data_final))]).argsort() # according to UD Levels
    data_final = np.array(data_final,dtype=object)
    data_final=np.concatenate(data_final[sort])
...
def ESort(i,data_final):
    ...
    if len(idxx)==0: return float(data_final[i][:,11][0])
