...
*   initialize the intigration coefficients on first entry
 ...
 200  continue
      ...
      in = m - NP + 1
      do 240 i = im,in
          dy(i) = b(1)*(v(i  )+v(i-1))+b(2)*(v(i+1)+v(i-2))
     &           +b(3)*(v(i+2)+v(i-3))+b(4)*(v(i+3)+v(i-4))
     &           +b(5)*(v(i+4)+v(i-5))
          dz(i) = b(1)*(w(i  )+w(i-1))+b(2)*(w(i+1)+w(i-2))
     &           +b(3)*(w(i+2)+w(i-3))+b(4)*(w(i+3)+w(i-4))
     &           +b(5)*(w(i+4)+w(i-5))
 240  continue
      do 260 i = im,in
         y(i) = y(i-1) + dy(i)
