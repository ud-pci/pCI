...
.. note::
	...
	``kval`` controls the values of :math:`\tilde{\epsilon}_v` in denominators :footcite:p:`2009AO`.
...
``second-cis`` is the fourth program of the all-order package. It calculates second-order MBPT corrections to the bare Hamiltonian for a much larger set of orbitals than the all-order codes, since high accuracy is not required for higher orbitals. It can be run on its own (CI+MBPT) or after the all-order codes, in which case it reads the all-order results and overlays them with MBPT coverage for the remaining orbitals. Such overlay of the all-order and MBPT corrections drastically improves the efficiency of the method.
