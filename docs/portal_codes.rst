...
In this section, we describe supplementary python scripts used to generate csv-formatted databases of energy levels and matrix elements for the University of Delaware's `Portal for High-Precision Atomic Data and Computation <https://www1.udel.edu/atom/>`_. The required input files can be obtained from ``pconf`` for energies and ``pdtm`` for matrix elements. This process has been heavily automated using the :doc:`pCI-py scripts <pci-py>`. 
