...
* The ``atom.isotope`` field specifies the isotope number. If left blank, the script will automatically find the atomic weight from the `IAEA nuclear charge radii table <https://www-nds.iaea.org/radii/>`_. This feature requires the ``atom.name`` field to contain a valid atomic symbol.
...
The ``ci.py`` script automatically generates the list of configurations given information about the atomic system of interest via a configuration file ``config.yml``. Note that if ``BASS.INP`` is not present, the order of the configurations will be by :math:`nl`, and not how it is defined in the basis set construction. This script reads the ``add`` block:
...
* The ``conf.parity.J_selection`` field defines whether the user wants energy levels of a specific :math:`J` value defined by ``J`` and ``JM``.
...
The ``gen_rates_lifetimes.py`` script generates csv-formatted datafiles of transition rates, transition properties, and lifetimes given output files from ``gen_portal_csv.py``. It is run with the atom name as a command-line argument:

.. code-block::

    python gen_rates_lifetimes.py <atom_name>

It expects the following input files to exist:

* ``{atom_name}_Energies.csv``
* ``{atom_name}_Matrix_Elements_Theory.csv``
* ``{atom_name}_Lifetimes_Exp.csv`` (optional) — experimental lifetimes to replace theory values

It generates the following output files:

* ``{atom_name}_Transition_Rates.csv``
* ``{atom_name}_Transition_Properties.csv``
* ``{atom_name}_Lifetimes.csv``
* ``{atom_name}_Transition_Rates_Error_Check.csv``
* ``{atom_name}_Lifetimes_Error_Check.csv``
* ``transitions.txt``
